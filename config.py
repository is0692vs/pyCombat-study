...
GAMMA = 0.99  # 将来の報酬をどれだけ重視するかを決定するパラメータ,1に近いと長期的な報酬を重視
EPSILON = 1.0  # 初期の探索率
EPSILON_MIN = 0.08 # 最小の探索率、0に近づけると、エージェントは最終的に探索を完全にやめ、最適な行動のみを取る
EPSILON_DECAY = 0.999  # 探索率の減衰率を緩やかにして、徐々に最適行動を学習
...
LEARNING_RATE = 0.0000001  # Qネットワークの重みを更新する速度,小さい値に設定すると、学習が遅くなりますが、安定した学習が期待できる
# 学習率スケジューラの設定
