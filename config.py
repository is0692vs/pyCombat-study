...
# 画面端のペナルティの設定
EDGE_PENALTY = -0.00  # 画面端にいる場合のペナルティを強化
...
GAMMA = 0.95  # 将来の報酬をどれだけ重視するかを決定するパラメータ,1に近いと長期的な報酬を重視
EPSILON = 1.0  # 初期の探索率
EPSILON_MIN = 0.05  # 最小の探索率、0に近づけると、エージェントは最終的に探索を完全にやめ、最適な行動のみを取る
EPSILON_DECAY = 0.95  # 探索率の減衰率を緩やかにして、徐々に最適行動を学習
